...
| `openbao-secrets-init` | 5 | Wait for an OpenBao EndpointSlice and run the health check from its local inventory node. Health must be 200 or 429. Need `OPENBAO_BOOTSTRAP_TOKEN`. Wait for and verify the active endpoint before enabling KV v2, file audit, Kubernetes auth, and policy `external-secrets-read`. Bind SA `external-secrets-openbao`. Assert all seeded secrets are non-empty and meet length/PEM requirements. Seed paths (`tags: openbao, bootstrap`). On first run, skips `authentik_admin_token` and prints a reminder. Run 2 (`--tags authentik-token`) PATCHes only `admin-token` into `secret/authentik` once the Authentik bootstrap admin exists. `end_role` if not ready. |
